...
class NewsFeedService {
    ...
    func getUser(completion: @escaping (UserReponse?) -> Void) {
    ...
    func revealPostIds(forPostId postdId: Int, completion: @escaping ([Int], FeedResponse) -> Void) {
        revealedPostIds.append(postdId)
        guard let feedResponse = self.feedResponse else { return }
        ...
        fetcher.getFeed(nextBatchFrom: newFromInProcess) { [weak self] (feed) in
            ...
            if self?.feedResponse == nil {
                ...
                if let oldProfiles = self?.feedResponse?.profiles {
                    let oldProflesFiltered = oldProfiles.filter({ (oldProfile) -> Bool in
                        !feed.profiles.contains(where: { $0.id == oldProfile.id })
                    })
                    profiles.append(contentsOf: oldProflesFiltered)
                }
